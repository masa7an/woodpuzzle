...
**バージョン**: 1.1（定義は `src/__init__.py` の `__version__`）  
**ジャンル**: はめ込み型シルエットパズル  
**技術**: Python + Pygame  
...
### 6-4. GA4イベント送信の再実装試行（中断）
**試行内容**: `platform.window.gtag`を直接使用してGA4イベント送信を再実装
- `analytics.py`を修正して`platform.window.gtag`を使用
- 遅延初期化、エラーハンドリング強化を実装
- `game.py`の全`analytics.send_event`呼び出しにエラーハンドリング追加

**結果**: クリア表示時にフリーズが発生（2025-12-19）
- `platform.window.gtag`の直接呼び出しでもフリーズが解消されず
- 原因は`gtag`呼び出し自体ではなく、他の要因の可能性もあり
- **中断**: フリーズ問題が解決するまで、イベント送信機能は無効化のまま

**現状**: PV計測のみ有効、カスタムイベント送信は無効
...
| 日付 | 内容 |
|------|------|
| 2026-07-17 | **ver 1.1 を公開**（https://masa7an.github.io/woodpuzzle/ ）<br>デプロイ時の知見（詳細は `CLAUDE.md`）:<br>- **pushしてもPagesのデプロイが走らないことがある**。1回目のpushでは20分待っても実行が0件、2回目のpushで即起動・成功。設定は無変更。原因未確定（ユーザー仮説: サインイン済みでないと動かない可能性）。対処は「もう一度push」<br>- `Deploy from a branch` 構成では `.github/workflows/` は**不要**（GitHub組み込みの `pages-build-deployment` が動く）<br>- 直下の `woodpazzule.apk` を退避せずビルドするとapkが自分自身を同梱しサイズが倍増する → `build_web.bat` で回避<br>- 公開版の新旧を人が判別できるよう、タブ/ウィンドウのタイトルにバージョンを表示<br>失われていたアセットを公開apkから復元（20ステージ/テキスト/フォント/効果音）<br><br>**ver 1.1 の内容**（初のバージョン付与。従来どこにもアプリのバージョンは無く、`build/version.txt` の 0.9.2 はPygbagのバージョン）<br>ソース一式をGit管理下に追加（それまでREADME/ROADMAP等しかコミットされていなかった）<br>コードレビューで確定したバグを修正:<br>- Web版でUndoをタップするとクラッシュ（存在しない `_undo()` を呼んでいた）<br>- Undo復元時に他ピースの占有セルを奪い盤面が壊れる<br>- エディタモードが毎フレーム例外（タプル要素数の不一致）<br>- ヒント(H)がドラッグ開始時に必ずOFFになり一度も表示されていなかった<br>- SPACEでプライバシーポリシーが閉じない（到達不能な重複分岐）<br>- 壊れた.stageファイルで起動不能／エディタ保存が既存ステージを上書き／ステージID解析でクラッシュ<br>リファクタリング:<br>- 重複削除（stage1定義の統合、グリッド構築の共通化、クリア処理の統合）<br>- 描画の軽量化（`valid_cells`をset化、ゴーストSurface・ピース色のキャッシュ化）<br>- 描画キャッシュの無効化を1箇所に集約<br>- ステージエディタを `src/editor.py` へ分離（game.py 1788→1434行）<br>- `REPO_MAP.md` 追加 |
| 2024-12-14 | 初版作成 |
| 2025-12-19 | フェーズ8完了: パフォーマンス最適化（もっさり感改善）<br>- 非同期処理の最適化（`clock.tick()`削除、重い処理の軽量化）<br>- オブジェクト生成の最適化（`pygame.Rect`削減、フォントキャッシュ化）<br>- 描画処理の最適化（グリッド描画の効率化、有効セルのみ描画）<br>- メモリ管理の改善（キャッシュクリア、メモリリーク対策）<br>- フェーズ3-1完了: ピース選択時の視覚フィードバック追加<br>- コードの重複削除とリファクタリング<br><br>フェーズ6-4: GA4イベント送信の再実装試行（中断）<br>- `platform.window.gtag`を使用した実装を試行<br>- クリア時にフリーズが発生したため中断 |
